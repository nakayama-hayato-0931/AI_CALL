...
## 新スキーマ (callcenter DB 採択版)

実装方針: callcenter の既存 `companies` テーブルを拡張して共有マスタとし、
fax-crm 固有カラムは `fax_customer_ext` (1:1) に分離する。

### `companies` (拡張済み)
既存 callcenter カラムに加え、fax-crm 互換のため以下を追加:
- `prefecture, city, postal_code, url, employee_count, representative`
- `note` (callcenter の `comment` とは別フィールド)
- `is_blacklisted, blacklisted_reason` (callcenter の `exclusion_flag` とは別軸)
- `source_file, imported_at`
- `external_faxcrm_id BIGINT UNSIGNED UNIQUE` (fax-crm.customers.id への逆参照)

### `fax_customer_ext` (fax-crm 固有、1:1)
`company_id (= companies.id)` を主キーに、`send_count, last_sent_at, last_pc_number, last_result, response_count` を保持。

### (参考) 旧設計の `customers` (廃案)
...
**採択: callcenter の MySQL を共有 DB として使う** (2026-06-01 方針変更後)

理由:
- callcenter は real-time 性が高い（lock 取得・call-list 取得・start/end が秒単位で発生）→ 内部ネットワークで <1ms に保つ必要
- 別プロジェクトの DB を参照すると public TCP 経由 (+10-50ms/クエリ) になり、1架電あたり +50-500ms の遅延が発生し得る
- fax-crm はバッチ性が高い（FAX送信、CSV取込、原稿管理など）ので、クロスプロジェクト遅延を吸収しやすい
- 「同期負荷をどちらが背負うか」で言うと、callcenter は内部 DB、fax-crm はリモート DB の方が運用上安全

旧採択 (fax-crm 側) は破棄。fax-crm DB に既に作成された `customers.region` / `customers.comment` / `callcenter_company_ext` は無害なので残置（Phase 4 で一括クリーンアップ）。
