...
export default function CallResultsPage() {
  ...
  // フィルター
  const [viewMode, setViewMode] = useState('daily'); // 'daily' | 'range' | 'all'
  ...
  const fetchCalls = async () => {
    ...
    try {
      ...
      if (onlyMine && user) params.user_id = user.id;
  ...
  const saveEdit = async (callId) => {
    try {
      await api.put(`/api/calls/${callId}/update`, editForm);
  ...
  return (
    <Layout>
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">架電結果</h1>
          <p className="text-sm text-gray-400 mt-0.5">過去の架電結果を確認・編集</p>
        </div>
      </div>
      ...
      <div className="card p-4 mb-4 space-y-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <div className={`w-9 h-5 rounded-full transition-colors relative ${onlyMine ? 'bg-blue-600' : 'bg-gray-300'}`}
              onClick={() => { setOnlyMine(!onlyMine); setOperatorId(''); setPage(1); }}>
              <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${onlyMine ? 'translate-x-4' : 'translate-x-0.5'}`} />
            </div>
            <span className="text-gray-600 text-xs font-medium">自分のみ</span>
          </label>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {/* ステータスフィルター */}
          <div>
            <select value={resultCode} onChange={e => { setResultCode(e.target.value); setPage(1); }} className="input text-sm">
          ...
          {!onlyMine && (
            <div>
              <select value={operatorId} onChange={e => { setOperatorId(e.target.value); setPage(1); }} className="input text-sm">
                      ...
                      <td className="table-cell">
                        {isEditing ? (
                          <select value={editForm.result_code} onChange={e => setEditForm({...editForm, result_code: e.target.value})}
                            ...
                            {RESULT_OPTIONS.filter(o => o.value).map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                          </select>
                        ) : (
                      ...
                      <td className="table-cell text-gray-400 text-xs max-w-xs">
                        {isEditing ? (
                          <input type="text" value={editForm.memo} onChange={e => setEditForm({...editForm, memo: e.target.value})}
                            className="input text-xs !py-1 !px-2 w-full" placeholder="メモ" />
                        ) : (
                      ...
                      <td className="table-cell text-center">
                        ...
                        {isEditing && (
                          <div className="flex items-center justify-center gap-1">
                            <button onClick={() => saveEdit(call.id)}
                              className="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700">保存</button>
                            <button onClick={() => setEditingId(null)}
                              className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded hover:bg-gray-200">取消</button>
                          </div>
                        )}
                      </td>
                    </tr>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-4">
          <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1}
            className="px-3 py-1 rounded text-sm bg-white text-gray-600 hover:bg-gray-100 disabled:opacity-40">←</button>
