/**
 * 案件詳細コンテンツ
 * - pages/projects/[id].js (URL直リンク) と pages/projects/index.js (一覧モーダル) の
 *   両方で使う共通コンポーネント。
 * - props.embedded=true でタイトル+「一覧に戻る」ボタンを隠す (モーダル側でヘッダ表示するため)。
 * - props.onSaved があればフォーム保存後に router.push(listUrl) せずに onSaved を呼ぶ
 *   (モーダル運用時に呼び元で一覧を再取得 + 閉じる)。
 */
...
export default function ProjectDetailContent({ id, onSaved, onClose, embedded = false }) {
  const { user } = useAuth();
