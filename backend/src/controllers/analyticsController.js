  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT
         ...
         CAST(SUM(CASE WHEN COALESCE(p.mail_sent, 0) = 0 AND COALESCE(p.phone_confirmed, 0) = 0 AND (p.status IS NULL OR p.status NOT IN ('LOST','SHORUI_CHU','SHORUI_OCHI','MODOSHI')) THEN 1 ELSE 0 END) AS SIGNED) as waiting_contact,
