...
const getQualityMetrics = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT
         COUNT(*) as total,
         CAST(SUM(CASE WHEN (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'lost' THEN 1 ELSE 0 END) AS SIGNED) as lost,
         CAST(SUM(CASE WHEN (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'waiting' THEN 1 ELSE 0 END) AS SIGNED) as waiting_contact,
         CAST(SUM(CASE WHEN (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'screening' THEN 1 ELSE 0 END) AS SIGNED) as screening_in_progress,
         CAST(SUM(CASE WHEN (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'iv_set' THEN 1 ELSE 0 END) AS SIGNED) as interview_set,
         CAST(SUM(CASE WHEN (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'iv_done' THEN 1 ELSE 0 END) AS SIGNED) as interview_done,
         CAST(SUM(CASE WHEN (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'barashi' THEN 1 ELSE 0 END) AS SIGNED) as barashi,
         CAST(SUM(CASE WHEN (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'other' THEN 1 ELSE 0 END) AS SIGNED) as other_status,
    ...
    const data = {
      ...
      otherStatus: Number(rows[0].other_status) || 0,
      otherStatusPct: pct(Number(rows[0].other_status) || 0),
    ...
    try {
      ...
      if (payrollRows.length > 0) {
        ...
        for (const [uid, total] of byUser) {
          if (total > 0) costMap.set(uid, total); // 給与PDFが優先（0は上書きしない＝打刻ベースの人件費を残す）
        }
      }
...
const getQualityAll = async (req, res, next) => {
  try {
    ...
    const fields = ['total','lost','waiting_contact','screening_in_progress','interview_set','interview_done','barashi','other_status','online_interview','no_screening','screening_failed'];
    ...
    if (qSystemFrom) {
      try {
        ...
        const adjustMap = {
          ...
          q_waiting_contact: { rowKey: 'waiting_contact', actualSql: `SELECT COUNT(*) as cnt FROM projects p WHERE p.owner_user_id = ? AND p.is_legacy = 0 AND p.is_prospect = 0 AND (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'waiting' AND DATE(p.created_at) = ?${wcSql}` },
          q_screening_in_progress: { rowKey: 'screening_in_progress', actualSql: `SELECT COUNT(*) as cnt FROM projects p WHERE p.owner_user_id = ? AND p.is_legacy = 0 AND p.is_prospect = 0 AND (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'screening' AND DATE(p.created_at) = ?${wcSql}` },
          q_interview_set: { rowKey: 'interview_set', actualSql: `SELECT COUNT(*) as cnt FROM projects p WHERE p.owner_user_id = ? AND p.is_legacy = 0 AND p.is_prospect = 0 AND (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'iv_set' AND DATE(p.created_at) = ?${wcSql}` },
          q_interview_done: { rowKey: 'interview_done', actualSql: `SELECT COUNT(*) as cnt FROM projects p WHERE p.owner_user_id = ? AND p.is_legacy = 0 AND p.is_prospect = 0 AND (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'iv_done' AND DATE(p.created_at) = ?${wcSql}` },
          ...
          q_other_status: { rowKey: 'other_status', actualSql: `SELECT COUNT(*) as cnt FROM projects p WHERE p.owner_user_id = ? AND p.is_legacy = 0 AND p.is_prospect = 0 AND (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'other' AND DATE(p.created_at) = ?${wcSql}` },
    ...
    const buildQ = (r) => {
      ...
      return {
        ...
        otherStatus: n('other_status'), otherStatusPct: p(n('other_status')),
    ...
    const [rows] = await pool.query(
      `SELECT p.id, p.job_number, p.status, p.created_at, p.interview_date, p.memo,
       ...
       WHERE p.is_legacy = 0 AND p.is_prospect = 0
         AND (CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'waiting'
...
const getScreeningInProgressDetail = async (req, res, next) => {
  try {
    ...
    const where = [
      ...
      `(CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'screening'`,
    ];
    const params = [];
...
/**
 * GET /api/analytics/interview-set-detail
 * 面接日確定 (interview_date IS NOT NULL かつ status が中間外/結果系) の案件明細。
 * getQualityMetrics の interview_set カウントと同じ条件で抽出する。
 * Query: date_from, date_to, user_id(任意=owner_user_id)
 * 列: 案件獲得日(created_at) / 求人番号 / 企業名 / 担当営業 / 架電担当(owner) /
 *     募集開始日 / 履歴書送付日 / 面接日 / ステータス
 */
const getInterviewSetDetail = async (req, res, next) => {
  try {
    const { date_from, date_to, user_id } = req.query;
    const { buildWorkCategoryFilter } = require('../middlewares/auth');
    const wcFilter = buildWorkCategoryFilter(req, 'p.work_category');
    ...
    const [rows] = await pool.query(
      `SELECT p.id, p.job_number, p.created_at AS acquired_at,
              ...
              su.name AS sales_name,
              ou.name AS caller_name,
              p.recruitment_start_date, p.resume_sent_date, p.interview_date, p.status
         FROM projects p
         LEFT JOIN companies c ON p.company_id = c.id
         LEFT JOIN users su ON p.sales_user_id = su.id
         LEFT JOIN users ou ON p.owner_user_id = ou.id
        WHERE ${where.join(' AND ')}
        ORDER BY p.interview_date ASC, p.created_at DESC, p.id DESC`,
      params
    );
    // ステータス内訳サマリ
    const statusCounts = {};
    rows.forEach(r => {
      const s = r.status || '面接実施待ち';
    ...
    logger.error(`[getInterviewSetDetail] ${err.message}`);
    return ApiResponse.error(res, err.message, 500);
  }
};
...
const getInterviewDoneDetail = async (req, res, next) => {
  try {
    ...
    const where = [
      `p.is_legacy = 0`,
      `p.is_prospect = 0`,
      `(CASE WHEN p.status = 'LOST' THEN 'lost' WHEN p.status = 'BARASHI' THEN 'barashi' WHEN p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI','HIRED','WAITING_RESULT','INTERVIEW_DONE') THEN 'iv_done' WHEN p.status = 'MENSETSU_KAKUTEI' OR (p.interview_date IS NOT NULL AND p.interview_date >= CURDATE() AND (p.mail_replied IS NOT NULL OR p.phone_confirmed IS NOT NULL)) THEN 'iv_set' WHEN p.status = 'SHORUI_CHU' OR (p.document_screening = 'required' AND p.status = 'BOSHUCHU') THEN 'screening' WHEN p.status = 'SHORUI_OCHI' THEN 'screening_failed' WHEN COALESCE(p.mail_replied,0) = 0 AND COALESCE(p.phone_confirmed,0) = 0 THEN 'waiting' ELSE 'other' END) = 'iv_done'`,
    ];
    const params = [];
    if (date_from) { where.push(`DATE(p.created_at) >= ?`); params.push(date_from); }
    if (date_to)   { where.push(`DATE(p.created_at) <= ?`); params.push(date_to); }
