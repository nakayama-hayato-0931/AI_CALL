...
const getInterviewSetDetail = async (req, res, next) => {
  try {
    ...
    const where = [
      `p.is_legacy = 0`,
      `p.is_prospect = 0`,
      `p.interview_date IS NOT NULL`,
      `(p.status IS NULL OR p.status NOT IN ('LOST','BARASHI','HORYU','MODOSHI','SHORUI_CHU','SHORUI_OCHI'))`,
      `(p.interview_date >= CURDATE() OR p.status IN ('NAITEI','FUGOKAKU','KEKKA_MACHI','NAITEI_TORIKESHI'))`,
    ];
    const params = [];
    if (date_from) { where.push(`DATE(p.created_at) >= ?`); params.push(date_from); }
    if (date_to)   { where.push(`DATE(p.created_at) <= ?`); params.push(date_to); }
