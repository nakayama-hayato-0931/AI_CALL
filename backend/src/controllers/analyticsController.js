  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT
         ...
         CAST(SUM(CASE WHEN p.interview_date IS NOT NULL AND (p.status IS NULL OR p.status NOT IN ('LOST','BARASHI','HORYU','MODOSHI')) THEN 1 ELSE 0 END) AS SIGNED) as interview_set,
    ...
    if (qSystemFrom) {
      try {
        ...
        const adjustMap = {
          ...
          q_interview_set: { rowKey: 'interview_set', actualSql: `SELECT COUNT(*) as cnt FROM projects p WHERE p.owner_user_id = ? AND p.is_legacy = 0 AND p.is_prospect = 0 AND p.interview_date IS NOT NULL AND (p.status IS NULL OR p.status NOT IN ('LOST','BARASHI','HORYU','MODOSHI')) AND DATE(p.created_at) = ?` },
