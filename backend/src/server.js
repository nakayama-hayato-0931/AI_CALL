...
// ヘルスチェック (Railway healthcheck 用、 即応答で DB 触らない)
// DB が詰まっていても Railway の healthcheck だけは必ず通す → デプロイが必ず成功する
app.get('/api/health', (req, res) => {
  res.json({ success: true, message: 'OK', timestamp: new Date().toISOString() });
});

// backend プロセス自体が生きているか確認用 (DB 触らない)
...
const criticalPreflight = async () => {
  ...
  // タイムゾーンは SQL 側で CONVERT_TZ(NOW(), '+00:00', '+09:00') する方針に変更したため
  // preflight での SET SESSION/GLOBAL time_zone は撤回 (healthcheck failure の原因のため)。
