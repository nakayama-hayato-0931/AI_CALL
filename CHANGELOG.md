...
- 実際の架電リスト xlsx の D 列 (電話番号) を調べたところ、 以下のような乱雑なセルが多数あった:
  - メール + 電話の混在 (`090-3109-5824\nh.d.k.hatake@adagio.ocn.ne.jp`)
  - 1 セルに 2〜3 件の電話 (`090-3907-7264\n092-432-7775` / `03-5615-2986 / 080-5387-8711 / 080-7833-1198`)
  - メールのみ (電話なしの行)、 数値型セル (int の `9055337733`)、 ハイフンなし 10/11 桁
  - 国際表記 (`+8180-4428-6082`)、 全角空白 / 全角カッコ内の番号、 連続改行 + 氏名 / 店舗名注記
- 旧ロジックの `extractPhoneNumbers` 正規表現は許容範囲が広く、 メール内のドット区切り文字列をマッチしてしまう恐れがあった。

#### 仕様
- 新ヘルパー `extractPhoneInfo(raw)` を csvController.js に追加: 戻り値 `{ primary, extras }`。
  - 1 セル内の全電話番号を抽出し、 先頭を primary、 2 件目以降を extras 配列で返す。
  - 採用条件: 数字 10 桁 (固定) or 11 桁 (携帯/フリーダイヤル) かつ先頭 '0'。 `+81xxx` (12〜13 桁で先頭 81) は先頭を '0' に置換して採用。
- 抽出アルゴリズム:
  1. NFKC 正規化で全角→半角統一 (全角数字・全角カッコ・全角空白)、 各種 Unicode ダッシュ → 半角 '-' に統一。
  2. 区切り文字 `\r \n , 、 ， ; ； / \ ( ) 空白 タブ` で token 分割 (ハイフン・ドットは番号内部に出るので区切らない)。
  3. token に '@' / `http(s)://` を含めばスキップ (メール・URL 除外)。
  4. 数字以外を全て除去し、 上記の桁数 + 先頭 0 条件に合致すれば採用。 同一番号は重複排除。
- 新ヘルパー `appendExtrasToComment(baseComment, extras)`: 2 件目以降を comment 末尾に `追加電話番号: 09012345678, 0312345678` の形で追記。 既に同じ文字列が含まれていれば二重追記しない。
- 適用箇所:
  - `importCompanies` (架電リスト一括 import): phone_number / fax_number セル両方に適用、 extras は comment に追記 (FAX extras は `FAX:` プレフィクス付き)。
  - `importSpecialList` (特別リスト import): 同上。
  - `manualAddCompany` / `manualAddSpecial` (1 件手動登録): ユーザーがメール混じり / 複数行で貼り付けたケースに対応、 primary を採用 + extras を comment に追記。
- 後方互換: `normalizePhoneNumber(raw)` は extractPhoneInfo の primary を返す薄いラッパとして残置 (他コントローラからの参照がある可能性に備える)。

#### 手動検証
- 実ファイル D 列 17 ケース (D2/D5/D11/D12/D25/D26/D27/D53/D64/D66/D80/D91/D161/D191 + 空/null/ユーザー指定検証ケース) で期待通りの primary + extras を抽出 (17/17 pass)。
- ユーザー指定検証: `extractPhoneInfo('03-1234-5678\n090-1234-5678 abc@example.com')` → `{ primary: '0312345678', extras: ['09012345678'] }` 確認。
- メールのみのセル (D26 `h-tanaka@f-million.com`) は primary=null となり、 既存の「企業名/電話番号必須」 スキップ経路に乗る (旧挙動と同等)。
- 数値型 int セル (D11 `9055337733`) は先頭 0 が欠落するため不採用 (旧データ修正で対応する方針、 ハードコード救済はしない)。

#### 影響範囲
- `backend/src/controllers/csvController.js` (extractPhoneNumbers 強化 / extractPhoneInfo 新設 / appendExtrasToComment 新設 / 4 entry-point への組込み)
- `CHANGELOG.md`
...
- 「6650件あるはずなのに、 他の小売りやその他サービス業などがピックアップされてしまう」
- 「出てこない。 架電ルールに無いから？」 の指摘。
- 業種別「農業」 ピックアップで、 業種カテゴリバッジが「建設」「飲食」「宿泊」 になっている企業が表示される事象。

#### 原因 (2 段階)
- 1 段目の旧仕様: `industry_category = '農業' OR industry_category IS NULL`
  - 未分類 (IS NULL) の企業 = 本当は小売・サービス業の企業を取り込み。
- 2 段目で顧客マスタと同じ `industry LIKE '%農業%'` を OR 追加 → やはり業種カテゴリバッジが「建設」「飲食」 の企業が出る。 これは **主業種は別だが industry テキストに副業として『農業』 を記述している企業** が拾われていた (顧客マスタの 6,650 件にも同じ理由で混入していた)。
- なお業種別モードでは `industry_region_rules` (架電ルール) は最初からバイパスされているため、 ルール設定の有無は本事象の原因ではない (companyController.js line 435 の `irFilter` 空文字化)。

#### 修正
- `companyController.js` 415 行 (getNextCallTarget) と 704 行 (getCallList) の `modeFilterSQL` を `industry_category` の **厳密一致のみ** に変更:
  ```sql
  AND c.industry_category = ?
  ```
- これでピックアップ対象 = 「主業種が農業に分類された企業」 のみ。 副業に農業が入っているだけの建設・飲食企業は除外される。

#### 運用上の前提
- 未分類 (industry_category IS NULL) の企業は業種別ピックアップで取れない。
- 顧客マスタの **業種診断ボタン → 「農業」 → 分類漏れ件数を確認 → industry_category 再計算** を実行することで分類漏れを解消する運用。
- 自由キーワード (`CATEGORY_NAMES_LIST` 外の業種名) を入力した場合は引き続き `industry LIKE '%キーワード%'` で部分一致 (変更なし)。
...
#### 修正 (フロント `pages/admin/customer-master.js`)
- `NG_REASON_OPTIONS` 定数 (11 種類) を追加 (`call.js` の NG結果入力フォームと同じ並び)。
- `filters` 初期値に `ng_reason: ''` を追加。 `clearAll` も同期 (ついでに region/show_excluded のクリア漏れも修正)。
- 結果ドロップダウンの右に「NG理由」 ドロップダウンを追加 (`filters.result === 'NG'` のときのみ表示)。 結果が NG 以外に変更されたら ng_reason も自動クリア。
- タイトル「顧客マスタ」 の右に件数表示 (`{Number(total).toLocaleString()}件`、 読込中は「読み込み中...」)。

#### 修正 (バックエンド `adminController.getCustomerMasterList` / `bulkSyncCustomers`)
- クエリパラメータ `ng_reason` を受け取り、 `callConds` に `cl.ng_reason = ?` を追加。
- `ng_reason` 単独で来た場合 (result 未指定) は `cl.result_code = 'NG'` を強制的に AND。 これにより別 result_code の通話に同名の理由が紛れることを防ぐ。
- bulkSyncCustomers でも同じフィルタ処理を追加 (一括同期もフィルタ反映)。

#### 結果
- 「結果=NG」 選択時のみ NG理由 ドロップダウンが表示され、 11 種類の理由でさらに絞り込み可能。
- タイトル「顧客マスタ {N,NNN}件」 でフィルタ適用後の総件数が一目で分かる。
...
- これまでは一覧から案件をクリックすると `/projects/[id]` へ画面遷移 → 編集 → 「一覧に戻る」 ボタンで戻る、 という流れだった。

#### 修正
- 共通コンポーネント `frontend/src/components/projects/ProjectDetailContent.jsx` を新規作成。
  従来の `pages/projects/[id].js` の中身 (企業情報・案件編集・通話履歴・内定者モーダル・不合格時面接人数モーダル) をそのまま移植し、 props に `embedded`/`onSaved`/`onClose` を追加。
- 3 つの一覧画面で、 行クリック時に画面遷移 (`router.push`) ではなく案件詳細モーダルを開くように変更:
  - `pages/projects/index.js` (オペレーター)
  - `pages/admin/projects.js` (管理者)
  - `pages/sales/projects.js` (営業)
- 各画面のモーダルは最大幅 `max-w-6xl`、 背景クリック・×ボタン・「更新する」 保存後で閉じる。 保存後は親の `fetchProjects()` を呼んで一覧を再取得。
- `pages/projects/[id].js` は短いラッパー (Layout + `<ProjectDetailContent id={id} />`) に圧縮。 URL 直リンクやブックマーク経由のアクセスは引き続き動く (`/projects/[id]` のサイズ 8.42 kB → 335 B)。
- 内定者情報モーダル・不合格モーダルは案件詳細モーダルの上に重なる用、 `z-[60]` に引き上げ (案件詳細自体は `z-40`)。

### 2026-06-18: NG 結果の企業を永久除外に変更
#### 背景
- 「架電結果が NG の企業が自動ピックアップされてしまう」 事象。
- 旧仕様: NG は最終架電から 3 ヶ月後 + 別オペレーターでのみ再ピックアップ可能 (`lastUserNotEqualSQL` + Tier 5 で実装)。
- 業務上は NG = もう架電したくないという運用方針なので、 ユーザー要望で永久除外に変更。

#### 修正
- `companyController.js` の `lastResultExclusionSQL` の result_code リストに `'NG'` を追加:
  ```sql
  AND cl2.result_code IN ('SKIP', 'PROJECT', 'RECALL', 'INTERESTED', 'NG')
  ```
- これに伴い Tier 5 (retry_ng) のクエリは事実上空配列を返すようになる (lrFilter で先に弾かれる)。 ロジック自体は残しておくが将来削除予定。
- `CLAUDE.md` のアーキテクチャ要点「架電優先度ロジック」 の優先順記述も更新。

### 2026-06-18 (追補): Tier 0 (assigned) も永久除外を適用 — SKIP も上がってこないように
#### 背景
- NG 永久除外を入れた直後、 ユーザーから 「SKIP の企業も上がってきていた」 報告。
- 調査の結果、 `getCallList` の **Tier 0 (assigned)** クエリ (自分割り当て中の企業を先頭に表示) に `${lrFilter}` が適用されていなかった。
- コメントには「永久除外 (SKIP/PROJECT/RECALL/INTERESTED)・業種地域フィルタ・last_called_at 経過日数条件をバイパス」 と書かれており、 これは **意図的なバイパス** だった (管理者が割り当てた企業は本人が必ず架電できるようにという設計)。
- しかし「SKIP/NG された企業は二度と再表示しない」 という今回の運用方針が優先されるため、 Tier 0 もバイパス対象から外す。

#### 修正
- `companyController.js` Tier 0 (assigned) クエリの WHERE 句に `${lrFilter}` を追加。
- コメントを 「永久除外をバイパス」 → 「永久除外は適用、 業種地域フィルタ・last_called_at 経過日数のみバイパス」 に更新。
- これで管理者が手動割り当て中の企業でも、 オペレーターが SKIP/NG した企業は Tier 0 に出てこなくなる。
- 他の Tier (2-5) は既に lrFilter 適用済みなので変更なし。

#### 既知の残課題 (今回未対応)
- Tier 1 (recall_due) は `recall_tasks` 由来のため `lrFilter` を適用していない。 「リコール設定 → SKIP」 した企業は recall_tasks が pending のまま残るので Tier 1 で出てくる可能性。 これは「SKIP 時に該当企業の pending recall_tasks をキャンセルする」 という別タスクで対応 (callController.js の end ハンドラ側で完結すべき)。 報告があれば対応。
